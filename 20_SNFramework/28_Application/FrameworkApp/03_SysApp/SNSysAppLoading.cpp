...
Void SNSysAppLoading::OnInitialize()
{
	...
	// 表示用シーケンサ設定
	TimerSeqDisp.Initialize(this, SeqChDisp, 6);
	TimerSeqDisp.SetWait(0, SeqChDispFadeTime);
	TimerSeqDisp.SetWait(1, SeqChDispPhaseTime);
	TimerSeqDisp.SetWait(2, SeqChDispFadeTime);
	TimerSeqDisp.SetWait(3, SeqChDispFadeTime);
	TimerSeqDisp.SetWait(4, SeqChDispPhaseTime);
	TimerSeqDisp.SetWait(5, SeqChDispFadeTime);
	// リソースロード
