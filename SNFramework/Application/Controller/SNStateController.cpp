...
//           -1:遷移なし
//           0~:状態クラス毎に規程する遷移先コード
SNTransitionCode SNStateController::Step(SNEvent* event)
{
	SNTransitionCode transition_code;
	UInt8 next_state;

	// 現在状態のStepを実行
	transition_code = ((SNScene*)SceneList.DirectAccess(NowState)->UserData)->Step(event);
...
// 描画処理
Void SNStateController::Draw(SNSurface* surface)
{
	// 表示状態のときのみ処理
	if (Visible)
	{
		...
		// 現在状態の描画処理を実行
		((SNScene*)SceneList.DirectAccess(NowState)->UserData)->Draw(surface);
	}
...
// 初期化
Void SNStateController::OnInitialize()
{
	return;
}

// 終了処理
Void SNStateController::OnTerminate()
{
	return;
}

// Entry
Void SNStateController::OnEntry()
{
	return;
}

// Exit
Void SNStateController::OnExit()
{
	return;
}
...
// 1フレーム実行
// リターン：遷移先コード
SNTransitionCode SNStateController::OnStep(SNEvent* event)
{
	return SNTransitionCodeStay;
}

// 描画処理(実装用)
Void SNStateController::OnDraw(SNSurface* surface)
{
	return;
}
