...
Void SNStorage::Initialize()
{
	...
	// リスト確保
	RequestList.Allocate(SNConfiguration::SystemConfiguration.StrageRequestMax);
...
Boolean SNStorage::SaveImageFile(String file_full, Handle image)
{
	...
	// PNG エンコーダの CLSID を取得
	if (GetEncoderClsid((String)L"image/png", (CLSID*)& clsid) == 0)
	{
		// 保存
		if (bmp.Save(file_full, &clsid, NULL) == Gdiplus::Ok)
		{
			ret = true;
		}
	}
...
// エンコーダー取得
Int SNStorage::GetEncoderClsid(String format, Void* clsid)
{
	Int ret = -1;
	UINT num = 0;
	UINT size = 0;
